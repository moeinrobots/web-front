...
export async function generateMetadata({params}:{params:Promise<{project:string}>}):Promise<Metadata>{const {project}=await params;const item=portfolio.find(p=>p.slug===project);return item?pageMetadata(`${item.title} | ${item.category}`,item.description||item.result,`/portfolio/${project}`,item.cover_url):{robots:{index:false,follow:false}}}
